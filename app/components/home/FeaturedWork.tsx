 ...
 */
function getColSpan(gallery: Gallery): string {
 ...
 */
function getDesktopAspect(gallery: Gallery): string {
  const size = gallery.displaySize;
...
export default function FeaturedWork({ galleries, quotes = [], title = "Featured Work" }: FeaturedWorkProps) {
  const { isMenuVisible } = useUI();

  // Build feed: galleries with interspersed quotes
  const feedItems = useMemo(() => {
    if (!galleries?.length) return [] as FeedItem[];
